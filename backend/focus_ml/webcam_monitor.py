...
class WebcamMonitor:
    ...
    def run(self):
        # Try multiple camera indices for MacOS compatibility
        cap = None
        print("[WebcamMonitor] Attempting to open camera...")
        for i in [0, 1, 2]:
            print(f"[WebcamMonitor] Trying camera index {i}...")
            try:
                cap = cv2.VideoCapture(i)
                if cap.isOpened():
                    ret, _ = cap.read()
                    if ret:
                        print(f"[WebcamMonitor] SUCCESS on camera index {i}")
                        break
                    else:
                        print(f"[WebcamMonitor] Camera {i} opened but read() failed")
                else:
                    print(f"[WebcamMonitor] Camera {i} failed to open")
            except Exception as e:
                print(f"[WebcamMonitor] Exception on camera {i}: {e}")
            if cap:
                cap.release()
            cap = None

        if cap is None or not cap.isOpened():
            print("[WebcamMonitor] ERROR: Could not open ANY webcam. Check:")
            print("  1. System Settings > Privacy & Security > Camera")
            print("  2. Make sure the app running Python (Terminal/iTerm/VS Code) has camera permission")
        ...
        while self.running:
            ...
            # Draw on frame
            # We skip imshow to keep it silent in the backend, or we could conditionally show it.
            # cv2.imshow("Study Monitor — Phase 1", frame)
            # if cv2.waitKey(1) & 0xFF == ord('q'):
            #     break
            time.sleep(0.03) # Cap framerate
