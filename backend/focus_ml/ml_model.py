...
def engineer_features(df):
    ...
    df["subject_difficulty"]   = df.get("Difficulty", pd.Series([3]*len(df)))
...
def get_subject_recommendations(model, df):
    ...
    assigned = {}
    subjects_to_recommend = df["Subject"].unique()
    for sub in sorted(subjects_to_recommend, key=lambda s: all_s[s][0][0] if s in all_s else 0, reverse=True):
        if sub not in all_s: continue
    ...
    results = sorted(
        [(s, format_hour(assigned[s][1]), assigned[s][0]) for s in assigned.keys()],
...
# ─────────────────────────────────────────────
RETRAIN_EVERY = 20        # retrain after every 20 new sessions
